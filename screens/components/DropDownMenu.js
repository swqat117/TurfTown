...
class DropdownMenu extends Component {
  ...
  renderActivityPanel() {
    if (this.state.activityIndex >= 0) {
      ...
      return (
        <View style={{position: 'absolute',flex:1, alignSelf: 'stretch', left: -64, right: -64, top: 40, bottom: 0}}>
    ...
    <View style={{opacity: 0.4, backgroundColor: 'black', flex: 1 }} />
      ...
      <ScrollView style={[{position: 'absolute', top: 0, left: 0, right: 0,flex:1, backgroundColor: 'white'}, heightStyle]} >
      
      
      {
        currentTitles.map((title, index) =>
        <TouchableOpacity key={index} activeOpacity={1} style={{flex: 1, height: 56,width:null}} onPress={this.itemOnPress.bind(this, index)} >
